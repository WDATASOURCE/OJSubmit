...
void bfs(int x, int y, int a[][220], char c){
    Point t, v;
...
int main(){
    while(scanf("%d%d", &n, &m) != EOF){
        ...
        */
        ...
        for(int i = 0; i < n; i++)
            for(int j = 0; j < m; j++)
                if(mp[i][j] == '@' && a[i][j] != -1 && b[i][j] != -1 && (long long)a[i][j] + (long long)b[i][j] < ans) 
                    ans = (long long)a[i][j] + b[i][j];
        printf("%I64d\n", ans * 11);
    }
    return 0;
}
