...
struct Hash{
    ...
    unsigned long long getHash(int l, int r){
        return (Has[r] - Has[l - 1] * nbase[r - l + 1]);
    }
}_hash;
    ...
    while(scanf("%d%d", &m, &l) != EOF){
        ...
        for(int i = 0; i < l; i++){
            ...
            for(int j = i; j < i + m * l; j += l){
                unsigned long long hv = _hash.getHash(j + 1, j + l);
                ma[hv]++;
            }
            if(ma.size() == m) ans++;
            for(int j = i; j < len; j += l){
                unsigned long long behv = _hash.getHash(j + 1, j + l);
                ma[behv]--;
                if(ma[behv] == 0) ma.erase(behv);
                if(j + l * (m + 1) - 1 >= len) break;
                unsigned long long hv = _hash.getHash(j + l * m + 1, j + l * (m + 1));
                ma[hv]++;
                if(ma.size() == m) ans++;
            }
        }
